defmodule DbmsProject3Web.ProjectControllerTest do
  use DbmsProject3Web.ConnCase

  alias DbmsProject3.StudentProjects

  @create_attrs %{annual_production_program: 42, classifier: "some classifier", notes: "some notes", project_defending_date: ~D[2010-04-17], task_given_date: ~D[2010-04-17], topic: "some topic"}
  ...
  def fixture(:project) do
    {:ok, project} = StudentProjects.create_project(@create_attrs)
    project
  end
  ...
  describe "update project" do
    ...
    test "redirects when data is valid", %{conn: conn, project: project} do
      ...
      assert html_response(conn, 200) =~ "some updated classifier"
    end
